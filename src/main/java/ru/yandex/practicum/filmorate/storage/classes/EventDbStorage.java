...
public class EventDbStorage implements EventStorage {
    ...
    public Event add(Event event) {
        String query = "INSERT INTO USER_EVENT (INSERT_DATE_TIME, USER_ID, EVENT_TYPE_ID, EVENT_OPERATION_ID, ENTITY_ID) " +
                " VALUES(? , ? , ? , ? , ?)";
        try {
    ...
    public List<Event> getUserEvent(Integer userId) {
        String query = "SELECT UE.EVENT_ID, UE.INSERT_DATE_TIME, UE.USER_ID,UE.ENTITY_ID, ET.NAME AS EVENT_TYPE_NAME, " +
                "EO.NAME AS EVENT_OPERATION_NAME " + "FROM USER_EVENT AS UE LEFT JOIN EVENT_TYPE AS ET ON UE.EVENT_TYPE_ID = ET.EVENT_TYPE_ID " +
                "LEFT JOIN EVENT_OPERATION AS EO " + "ON UE.EVENT_OPERATION_ID = EO.EVENT_OPERATION_ID " + "WHERE UE.USER_ID=?";
        try {
    ...
    private Event makeEvent(ResultSet rs, int i) throws SQLException {
        try {
            ...
            throw new SQLErrorTransaction("Не удалось создать объект события на основе базы данных");
        }
    }
}
